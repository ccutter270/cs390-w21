...
; Create random (but connected) undirected, unweighted graphs.
; Perform both a standard and an agent-based BFS on the graph
; ----------------
; A bfs requires a queue, also called a FIFI list. The queue is implemented using a list.
; An empty queue is an empty list []
; To insert x into queue Q:   Q = lput x Q
; To get the first item in the queue:  x = first Q
; To remove the first item from the queue: Q = but-first Q.  (This returns the updated Q, not the item removed).
; To check if the queue is empty, check if length Q = 0.
...
links-own [ tree-edge? ]                         ; set to true if this link is an edge in a search tree
searchers-own [
...
; globals
globals [
  increments                                     ; number of steps for a searcher to traverse an edge. controls visualization speed
]
...
to bfs [ root ]
  ...
  ; To start search, visit the root and put it at the front of the queue
  visit root
  set Q lput root Q

  ; Continue processing the vertex at the front of the queue until the queue is empty
  while [ length Q > 0 ][
    let v first Q                ; takes two lines to get the front item off the queue and remove it from the queue
    set Q but-first Q
    ask [link-neighbors] of v [  ; every unvisited neighbor vertex gets visited and added to the queue
      if not visited? [
        ; visit the vertex, show that the edge was followed, and add the vertex to the front of the queue
        be-visited
        set parent v
        ask link-with v [set color red set thickness .25 set tree-edge? true]
        set Q lput self Q
      ]
    ]
  ]
  ...
  if visualize-bfs-tree? [ layout-radial vertices links with [tree-edge?] root ]
end
...
to launch-searcher-toward-me-from [ v ]
    hatch-searchers 1 [
      ...
      face destination
      set speed (distance destination) / increments
    ]
end
...
to move-searchers
  repeat increments [ ask searchers [fd speed] ]
...
; ================ SEARCHER CONTEXT HELPER PROCEDURES FOR GRAPH TRAVERSAL =============

; this moves a given agent to the location of vertex v more slowly,
; so that movement can be seen without changing the speed slider.
to travel-to [ v ]
  face v
  let steps distance v / speed
  repeat steps [
    ifelse distance v > speed [ fd speed ] [ fd distance v  ]
  ]
end
