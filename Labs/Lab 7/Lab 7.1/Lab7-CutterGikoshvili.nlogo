...
to setup
  ...
  create-fireflies num-fireflies [

    setxy random-xcor random-ycor
    set color white
    set shape "bug"
    set clock random (delay + 1)
    set flashing? false
  ]
...
to go-until-synched
  ...
  while [any? fireflies with [ not flashing? ] ] [ go ]
...
to go
  ...
  ask fireflies [
      ...
      if any? other fireflies with [flashing?] in-radius vision-radius [
          set clock (delay - 1 )
       ]
  ]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if (count fireflies != 0 ) [\nplot ((count fireflies with [flashing?] ) / (count fireflies))\n]"
...
<experiments>
  <experiment name="experiment" repetitions="3" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>count fireflies with [flashing?] &gt;= count fireflies</exitCondition>
    <metric>ticks</metric>
    ...
    <enumeratedValueSet variable="num-fireflies">
      <value value="100"/>
      <value value="500"/>
      <value value="1000"/>
    </enumeratedValueSet>
